...
**Huge Problem(Pain Point):** 여러 작업자가 44개 감정을 멀티라벨링하면 기준 해석 차이와 저장 누락이 데이터 품질을 흔든다.

**솔루션 한 줄 정의:** 공통 라벨 체계·작업자 흐름·건별 자동저장으로 협업 라벨링을 표준화한다.

**현재 상태:** 협업 라벨링 도구

**문제 해결 중심의 사고 흐름**

1. **관찰** — 다중 감정 라벨은 선택지가 많고 작업자가 바뀔 때 진행 상태와 기준을 일관되게 유지하기 어렵다.
2. **선택** — 모델 추천은 기본적으로 숨겨 gold set 오염을 줄이고, 사람·기사 단위로 즉시 저장하게 했다.
3. **구현** — React 라벨링 UI, Firebase/SQLite 저장 경로, 44개 라벨 정본과 데이터 임포트 도구를 구성했다.
4. **검증과 한계** — 배포 앱과 빌드 절차, 라벨 플래그가 문서화되어 있다. 작업자 간 일치도 지표는 별도 산출이 필요하다.
