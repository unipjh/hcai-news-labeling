...
import { GROUPS, LABELS, NONE_LABEL, SKIP_REASONS } from "./labels.js";
...
export async function saveAnnotation(payload) {
  try {
    ...
    const refreshed = await getQueue(payload.annotator_id);
    return { ok: true, progress: refreshed.progress };
  } catch (error) {
